<template>
  <div>
    <div class="header">
      <div class="header__left">
        <img src="/src/assets/images.jfif" alt="" />
        <div class="header__input">
          <span class="material-icons"> search </span>
          <input type="text" placeholder="Search Facebook" id="search_name" />
        </div>
        <div class="header__input">
          <button class="search" v-on:click="search">Search</button>
          <!-- <span class="material-icons"> search </span>
                <input type="text" placeholder="Search Facebook" /> -->
        </div>
      </div>

      <div class="header__right">
        <router-link to="/feed">
          <button class="btns">Feed</button>
        </router-link>
        <button class="btns red" v-on:click="logout"><i class="fa fa-sign-out" style="font-size:15px"></i>  Log Out</button>
        <div class="header__info">
          <router-link :to="'/' + this.user_id + '/profile'"
            ><div class="header__info">
              <img
                class="user__avatar"
                src="data:image/png;base64,"
                alt=""
              />
              <h4>{{ username }}</h4>
            </div></router-link
          >
        </div>

       
      </div>
    </div>
    <div class="max-height " >
    <!-- <div class="row-c">
        <h1 style="text-align:center;margin-top:1em; color:black" >Followers</h1>
      <div class="column-u" v-for="(user, index) in my_follower" :key="index">
        <div class="cards">
          <img
            :src="user.dp[0].dp_name"
            alt="Avatar"
            style=""
            v-if="user.dp[0].dp_name"
          />
          <div class="container-u">
            <router-link :to="'/' + user.user_id + '/profile_frnd'"
              ><h4>
                <b>{{ user.firstname }} {{ user.lastname }}</b>
              </h4>
            </router-link>

            <p class="fu">
              <button class="he">All Post : {{ user.posts.length }}</button>
              <button class="he">Followers :{{ user.followed.length }}</button>
              <button class="he">Following :{{ user.followers.length }}</button>
            </p>
            <hr />
            <p class="fu">
              
              <button class="button" v-on:click="unfolloow(user.user_id)">
                unfollow
              </button>
            </p>
          </div>
        </div>
      </div>
      </div>
      <div></div> -->
     
    <div>
     <div class="row-f">
      <div><h1 style="margin-top:25px;text-align:center; color:black" >Following</h1>
      </div>
      <div class="column-f" style="" v-for="(user, index) in my_follower" :key="index">
        <div class="cards">
          <img
            :src="user.dp[0].dp_name"
            alt="Avatar"
            style=""
            v-if="user.dp[0].dp_name"
          />
          <div class="container-u" >
            <router-link :to="'/' + user.user_id + '/profile_frnd'"
            style="text-decoration: none; color: rgb(7, 7, 171); "><h4>
                <b>{{ user.firstname }} {{ user.lastname }}</b>
              </h4>
            </router-link>

            <p class="fu">
              <button class="he">All Post : {{ user.posts.length }}</button>
              <button class="he">Followers :{{ user.followed.length }}</button>
              <button class="he">Following :{{ user.followers.length }}</button>
            </p>
            <hr />
            <p class="fu">
              <button class="button" v-on:click="unfolloow(user.user_id)">
                unfollow
              </button>
              
            </p>
          </div>
        </div>
      </div>
      
    </div>
     <div class="row-fr">
      <div><h1 style="margin-top:25px;text-align:center; color:black" >Followers</h1>
      </div>
      <div class="column-f" style="background-color:;" v-for="(user, index) in my_following" :key="index">
        <div class="cards">
          <img
            :src="user.dp[0].dp_name"
            alt="Avatar"
            style=""
            v-if="user.dp[0].dp_name"
          />
          <div class="container-u" >
            <router-link :to="'/' + user.user_id + '/profile_frnd'"
            style="text-decoration: none;color: rgb(7, 7, 171); "><h4>
                <b>{{ user.firstname }} {{ user.lastname }}</b>
              </h4>
            </router-link>

            <p class="fu">
              <button class="he">All Post : {{ user.posts.length }}</button>
              <button class="he">Followers :{{ user.followed.length }}</button>
              <button class="he">Following :{{ user.followers.length }}</button>
            </p>
            <hr />
            <p class="fu">
              <button class="button" v-on:click="folloow(user.user_id)">
                Follow
              </button>
              
            </p>
          </div>
        </div>
      </div>
      
    </div>
  </div>
       <!-- <div><h1 style="margin-top:370px;text-align:center; color:black" >Following</h1>
      </div>
      <div class="row-c" style="widht:100%; " >
          <div class="column-u" v-for="(user, index) in my_following" :key="index">
        <div class="cards">
          <img
            :src="user.dp[0].dp_name"
            alt="Avatar"
            style=""
            v-if="user.dp[0].dp_name"
          />
          <div class="container-u">
            <router-link :to="'/' + user.user_id + '/profile_frnd'"
              ><h4>
                <b>{{ user.firstname }} {{ user.lastname }}</b>
              </h4>
            </router-link>

            <p class="fu">
              <button class="he">All Post : {{ user.posts.length }}</button>
              <button class="he">Followers :{{ user.followed.length }}</button>
              <button class="he">Following :{{ user.followers.length }}</button>
            </p>
            <hr />
            <p class="fu">
              <button class="button" v-on:click="folloow(user.user_id)">
                Follow
              </button>
              
            </p>
          </div>
        </div>
      </div> -->
    <!-- </div> -->


  </div>
</div>
</template>

<script>
export default {
  name: "all_user",
  data() {
    return {
      isActive: false,
      user_id: JSON.parse(localStorage.getItem("user_info")).user_id,
      username:
        JSON.parse(localStorage.getItem("user_info")).firstname +
        " " +
        JSON.parse(localStorage.getItem("user_info")).lastname,
      users: [],
      my_following: [],
      my_follower: [],
      link: "https://devbook-t6cy.onrender.com",
    };
  },
  methods: {
    logout() {
      // this.$store.commit('token_mutate', "");
      localStorage.clear();
      this.$router.replace({ name: "Login" });
    },
    all_user() {
      fetch(`${this.link}/api/users`, {
        method: "GET",
        headers: { sar_babu: localStorage.getItem("token") },
      })
        .then((res) => {
          if (res.ok) {
            return res.json();
          } else if (res.status === 401) {
            throw new Error();
          }
        })
        .then((res) => {
          // (this.email_id = res.email_id),
          // (this.username = res.username),
          // (this.password = res.password),
          // (this.user_id = res.user_id),
          // (this.posts.push(res));
          this.users = res;
          // this.posts[0] = this.posts[0].reverse();
          console.log(res);
        })
        .catch((err) => this.$router.replace({ name: "Login" }));
    },
    folloow(id) {
      console.log(id);
      fetch(`https://devbook-t6cy.onrender.com/follow/${this.user_id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          // "Access-Control-Allow-Origin": window.location.origin,
          sar_babu: localStorage.getItem("token"),
        },

        body: JSON.stringify({
          follower_id: this.user_id,
          followed_id: id,
        }),
      })
        .then((res) => {
          if (res.ok) {
            return res.json();
          } else if (res.status === 401) {
            throw new Error();
            // console.log(res);
          } else {
            // console.log(res);
          }
        })
        .then((data) => {
          console.log(data);
          this.all_user();
          this.followers();
          this.following();
          // this.$router.replace({ path: "/users" });
        })
        .catch((err) => console.log(err));
    },
    unfolloow(id) {
      fetch(`https://devbook-t6cy.onrender.com/unfollow/${this.user_id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          // "Access-Control-Allow-Origin": window.location.origin,
          sar_babu: localStorage.getItem("token"),
        },

        body: JSON.stringify({
          follower_id: this.user_id,
          followed_id: id,
        }),
      })
        .then((res) => {
          if (res.ok) {
            return res.json();
          } else if (res.status === 401) {
            throw new Error();
            // console.log(res);
          } else {
            // console.log(res);
          }
        })
        .then((data) => {
          console.log(data);
          this.followers();
          this.following();
          //  this.$router.replace({ path: "/users" })
          //    this.myfollowerslength = data.length;
          //    this.myfollowers = data
          // this.get_user_details();
          //    this.$router.push({ path: 'profile' });
        })
        .catch((err) => console.log(err));
    },
    search() {
      var name = document.querySelector("#search_name");
      console.log(name.value);
      this.$router.push({ path: `/${name.value}/search_user` });
    },
    followers() {
      fetch(`https://devbook-t6cy.onrender.com/myfollowername/${this.user_id}`, {
        method: "GET",
        headers: {
          sar_babu: localStorage.getItem("token"),
        },
      })
        .then((res) => {
          if (res.ok) {
            return res.json();
          } else if (res.status === 401) {
            throw new Error();
            // console.log(res);
          } else {
            // console.log(res);
          }
        })
        .then((data) => {
          console.log(data);
          this.my_follower = data;
          // this.all_user();
          // this.$router.replace({ path: "/users" });
        })
        .catch((err) => console.log(err));
    },
    following() {
      fetch(`https://devbook-t6cy.onrender.com/myfollowingname/${this.user_id}`, {
        method: "GET",
        headers: {
          sar_babu: localStorage.getItem("token"),
        },
      })
        .then((res) => {
          if (res.ok) {
            return res.json();
          } else if (res.status === 401) {
            throw new Error();
            // console.log(res);
          } else {
            // console.log(res);
          }
        })
        .then((data) => {
          console.log(data);
          this.my_following = data;
          // this.all_user();
          // this.$router.replace({ path: "/users" });
        })
        .catch((err) => console.log(err));
    },
  },
  mounted() {
    this.all_user();
    this.followers();
    this.following();
  },
};
</script>
<style>

.column-f {
  float: left;
  max-width:350px;
  padding: 10px;
  height: 400px; /* Should be removed. Only for demonstration */
}
.row-f{
  width:49%;
  display:inline-block;
  overflow:scroll;
  max-height:600px;
  background-color:aliceblue
 
}
.row-fr{
  
  width:49%;
  overflow:scroll;
  background-color:aliceblue;
  float:right;
  max-height:600px;
}

/* Clear floats after the columns */
.row-f:after {
  content: "";
  display: table;
  clear: both;
}
.row-fr:after {
  content: "";
  display: table;
  clear: both;
}
.column-u {
  float: left;
  width: 25%;
  padding: 0 10px;
}

.cards:hover {
  box-shadow: 0 8px 16px 0 rgba(0, 0, 0, 0.2);
}
.vl {
    border-left: 6px solid green;
    height: 00px;
    margin-left:50%;
  }




.max-height{
    
  
}

/* Clear floats after the columns */




/* Style the counter cards */



img {
  border-radius: 5px 5px 0 0;
}
</style>
